...
export function ProjectList() {
  ...
  const [isChangingVisibility, setIsChangingVisibility] = useState<string | null>(null);

  // Toggle Visibility Handler
  const handleToggleVisibility = async (project: Project, newVisibility: "public" | "private") => {
    if (project.visibility === newVisibility) {
      setOpenMenuProjectId(null);
      return;
    }
    setIsChangingVisibility(project._id);
    try {
  ...
  // Execute Project Deletion Handler
  const handleExecuteDelete = async () => {
    if (!deleteTargetProject) return;
    setIsDeleting(true);
    setDeleteError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/api/projects/${deleteTargetProject._id}`, {
        method: "DELETE",
        credentials: "include",
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => {
            ...
            const isOwner =
              user &&
              project.owner &&
              (project.owner._id === user._id || (project.owner as any)._id === (user as any).id);
                    ...
                    <div className="flex items-center space-x-1.5 shrink-0">
                      ...
                      >
                        {project.visibility === "private" ? (
                          ...
                          <span className="flex items-center gap-1">
                            <Globe className="w-2.5 h-2.5 text-emerald-600" /> Public
                          </span>
                        )}
                      </Badge>
                            ...
                            >
                              ...
                              <div className="px-3 py-1 space-y-1">
                                ...
                                <div className="grid grid-cols-2 gap-1 bg-slate-50 p-1 rounded-xl border border-slate-100">
                                  <button
                                    disabled={isChangingVisibility === project._id}
                                    onClick={() => handleToggleVisibility(project, "private")}
                                  ...
                                  <button
                                    disabled={isChangingVisibility === project._id}
                                    onClick={() => handleToggleVisibility(project, "public")}
                                    className={`flex items-center justify-center space-x-1 py-1 rounded-lg text-[11px] font-medium transition-all ${
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-md w-full p-6 space-y-6 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start space-x-4">
              ...
              <div className="space-y-1 min-w-0 flex-1">
                <h3 className="text-lg font-bold text-slate-900">
                  Delete {deleteTargetProject.name}?
                </h3>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-2">
              <p className="text-xs font-semibold text-slate-700">
                To confirm deletion, type <span className="font-mono bg-white px-1.5 py-0.5 rounded border border-slate-200 text-slate-900 font-bold">{deleteTargetProject.name}</span> in the box below:
              </p>
              <input
                type="text"
                value={confirmDeleteName}
                onChange={(e) => setConfirmDeleteName(e.target.value)}
                placeholder={`Type ${deleteTargetProject.name}`}
            ...
            <div className="flex items-center justify-end space-x-3 pt-2">
              ...
              <Button
                size="sm"
                disabled={confirmDeleteName !== deleteTargetProject.name || isDeleting}
